...
contract NFTBasket is minting_1 {  
  ...
  constructor (address AXNFT) {   //so that we can use always the current address of the AXNFT Smart Contract
  ...
  //before that you must give a approve to the Smart Contract and to the executor of the function because it controls about that 
  ...
  function packing_Basket_1(uint256 tokenId, address _from) public {
    ...
    NFTContract.transferFrom(_from, address(this), tokenId); //sending the NFTs
  ...
  function unpack_NFT_1(address from, uint256 amount) public {
    //initialize the variables
    string memory aXedrasId;
    string memory finessbar;
    uint256 weightbar;
  ...
  function get_specificNFT_1(uint tokenId, uint amount, address from) public {
    //initialize the variables
    string memory aXedrasId;
    string memory finessbar;
    uint256 weightbar;
